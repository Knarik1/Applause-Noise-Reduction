...
mix_path = "./data/Songs/test_song/Carlos Gonzalez - A Place For Us/mixture16_noisy_song.wav"
song_path = "./data/Songs/test_song/Carlos Gonzalez - A Place For Us/mixture16.wav"
real_sr = 16000
...
for i in range(big_frames_count):
    ...
    for j in range(small_frames_count):
        ...
        #formula
        # cur_ratio_mask = np.clip(np.abs(cur_song_ft)/ (np.abs(cur_mix_ft) + 1e-7), 0, 3)
        cur_ratio_mask_appl = np.clip(np.abs(cur_mix_ft - cur_song_ft) / (np.abs(cur_mix_ft)+ 1e-7), 0, 3)
...
wavfile.write("./ideal_test_clip_3_appl.wav", real_sr, new_song.astype("int16"))
